...
st.set_page_config(page_title="ACC Setup Master v9.13", layout="wide")

# Custom CSS voor een professionele race-look
st.markdown("""
    <style>
...
cars_db = {
    ...
    "McLaren 720S EVO": {"bb": 53.2, "diff": 70, "steer": 13.0, "wr_f": 155, "wr_r": 125, "f_cam": -3.5, "r_cam": -3.0, "f_toe": 0.06, "r_toe": 0.10, "caster": 12.0, "tips": "Houd rijhoogte stabiel."},
...
# 3. SELECTIE
st.title("🏎️ :red[ACC] Setup Master v9.13")
st.markdown("---")
...
ukey = f"v913_{auto}_{circuit}".replace(" ", "_").replace("-", "")

# 4. SIDEBAR
st.sidebar.header("🩺 De Setup Dokter")
klacht = st.sidebar.selectbox("Wat voel je?", ["Geen", "Onderstuur", "Overstuur"], key=f"dr_{ukey}")
if "Onderstuur" in klacht: st.sidebar.error("Verlaag Front ARB")
elif "Overstuur" in klacht: st.sidebar.warning("Verlaag Rear ARB")
st.sidebar.divider()
...
with tabs[0]:
    ...
    m2.metric("Camber F", f"{car['f_cam']}°")
    m3.metric("Caster", f"{car['caster']}°")
    st.divider()
...
with tabs[1]:
    st.write("### :orange[Electronics]")
    st.slider("TC1", 0, 11, 3, key=f"tc1_{ukey}")
    st.slider("ABS", 0, 11, 3, key=f"abs_{ukey}")

with tabs[2]:
    st.write("### :green[Fuel & Brakes]")
    st.text_input("Fuel", "62", key=f"f_{ukey}")
    st.text_input("Brake Duct Front", bduct, key=f"bdf_{ukey}")

with tabs[3]:
    st.write("### :violet[Mechanical]")
...
with col_btn1:
    save_btn = st.button("💾 Sla Setup op", use_container_width=True)
