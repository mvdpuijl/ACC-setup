...
# 1. Configuratie
st.set_page_config(page_title="ACC Setup Master v7.1", layout="wide")
...
# 3. Selectie
st.title("🏎️ ACC Setup Master v7.1")
...
# 4. Tabs (Nu overal de werkende Mechanical Grip-methode!)
tabs = st.tabs(["🛞 Tyres", "⚡ Electronics", "⛽ Fuel & Strategy", "⚙️ Mechanical Grip", "☁️ Dampers", "✈️ Aero"])

with tabs[0]: # TYRES (Nu met dynamische waarde + ukey)
    st.subheader("Tyres & Alignment")
    t1, t2 = st.columns(2)
    with t1:
        st.write("**Front**")
        st.text_input("LF PSI", str(psi_v), key=f"psi_lf_{ukey}")
        st.text_input("RF PSI", str(psi_v), key=f"psi_rf_{ukey}")
        st.text_input("LF Toe", "0.06", key=f"toe_lf_{ukey}")
        st.text_input("LF Camber", "-3.5", key=f"cam_lf_{ukey}")
    with t2:
        st.write("**Rear**")
        st.text_input("LR PSI", str(psi_v), key=f"psi_lr_{ukey}")
        st.text_input("RR PSI", str(psi_v), key=f"psi_rr_{ukey}")
...
with tabs[3]: # MECHANICAL GRIP (De methode die al werkte)
    st.subheader("Mechanical Grip")
    m1, m2 = st.columns(2)
    with m1:
        st.write("**Front Settings**")
        st.text_input("Front Anti-roll bar", str(arb_f_v), key=f"farb_{ukey}")
    ...
    with m2:
        ...
        st.text_input("Rear Anti-roll bar", str(arb_r_v), key=f"rarb_{ukey}")
...
with tabs[4]: # DAMPERS (Nu ook dynamisch!)
    st.subheader("Dampers (B / FB / R / FR)")
    d1, d2, d3, d4 = st.columns(4)
    with d1:
        st.write("**LF**")
        st.number_input("B LF", 0, 40, int(d_vals[0]), key=f"b_lf_{ukey}")
        st.number_input("FB LF", 0, 40, int(d_vals[1]), key=f"fb_lf_{ukey}")
        st.number_input("R LF", 0, 40, int(d_vals[2]), key=f"r_lf_{ukey}")
        st.number_input("FR LF", 0, 40, int(d_vals[3]), key=f"fr_lf_{ukey}")
    with d2:
        st.write("**RF**")
        st.number_input("B RF", 0, 40, int(d_vals[0]), key=f"b_rf_{ukey}")
        st.number_input("FB RF", 0, 40, int(d_vals[1]), key=f"fb_rf_{ukey}")
        st.number_input("R RF", 0, 40, int(d_vals[2]), key=f"r_rf_{ukey}")
        st.number_input("FR RF", 0, 40, int(d_vals[3]), key=f"fr_rf_{ukey}")
    with d3:
        st.write("**LR**")
        st.number_input("B LR", 0, 40, int(d_vals[0]), key=f"b_lr_{ukey}")
        st.number_input("FB LR", 0, 40, int(d_vals[1]), key=f"fb_lr_{ukey}")
        st.number_input("R LR", 0, 40, int(d_vals[2]), key=f"r_lr_{ukey}")
        st.number_input("FR LR", 0, 40, int(d_vals[3]), key=f"fr_lr_{ukey}")
    with d4:
        st.write("**RR**")
        st.number_input("B RR", 0, 40, int(d_vals[0]), key=f"b_rr_{ukey}")
        st.number_input("FB RR", 0, 40, int(d_vals[1]), key=f"fb_rr_{ukey}")
        st.number_input("R RR", 0, 40, int(d_vals[2]), key=f"r_rr_{ukey}")
        st.number_input("FR RR", 0, 40, int(d_vals[3]), key=f"fr_rr_{ukey}")
...
with tabs[5]: # AERO
    ...
    with a2:
        ...
        st.number_input("Rear Wing", 0, 20, int(wing_v), key=f"wing_{ukey}")
